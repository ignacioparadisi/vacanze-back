    ...
    public static class RestaurantRepository
    {
        public static long AddRestaurant(Restaurant restaurant)
        {
            var table = PgConnection.Instance.ExecuteFunction(
                ...
                restaurant.Name, restaurant.Capacity, restaurant.IsActive, restaurant.Specialty, restaurant.Price, restaurant.BusinessName, 
        ...
        public static List<Restaurant> GetRestaurants()
        {
            ...
            for (var i = 0; i < table.Rows.Count; i++)
            {
                ...
                var specialty = table.Rows[i][4].ToString();
                var price = Convert.ToInt32(table.Rows[i][5]);
                var businessName = table.Rows[i][6].ToString();
                var picture = table.Rows[i][7].ToString();
                var description = table.Rows[i][8].ToString();
                var phone = table.Rows[i][9].ToString();
                var location = Convert.ToInt32(table.Rows[i][10]);
                var address = table.Rows[i][11].ToString();
