...
CREATE OR REPLACE FUNCTION public.addflight(_plane integer,
...
BEGIN
    ...
    VALUES (_price, TO_TIMESTAMP(_departure, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone,
            TO_TIMESTAMP(_arrival, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone, _plane, _loc_departure,
            _loc_arrival) RETURNING fli_id into _fli_id;
    RETURN _fli_id;
END;
...
CREATE OR REPLACE FUNCTION public.updateflight(_id integer,
                                               _plane integer,
                                               _price double precision,
                                               _departure character varying,
                                               _arrival character varying,
...
CREATE OR REPLACE FUNCTION public.deleteflight(_id integer)
    RETURNS integer
    LANGUAGE 'plpgsql'
...
CREATE OR REPLACE FUNCTION public.getflightsbydate(_begin char varying,
                                                   _end char varying)
    RETURNS TABLE
            (
                id            integer,
...
BEGIN
    RETURN QUERY SELECT fli_id,
                        ...
                        fli_loc_arrival
                 FROM public.Flight
                 WHERE fli_departuredate BETWEEN TO_TIMESTAMP(_begin, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone AND
                               TO_TIMESTAMP(_end, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone +
                               '1 days'::interval;
END;
...
CREATE OR REPLACE FUNCTION public.getOutBoundFlights(_departure integer,
                                                     _arrival integer,
...
BEGIN
    RETURN QUERY SELECT fli_id,
                        ...
                        fli_loc_arrival
                 FROM public.Flight
                 WHERE fli_departuredate::date = _departuredate::timestamp without time zone
                   and fli_loc_departure = _departure
                   and fli_loc_arrival = _arrival;
END;
...
CREATE OR REPLACE FUNCTION public.getRounTripFlights(_departure integer,
                                                     _arrival integer,
                                                     _departuredate char varying,
...
BEGIN
    RETURN QUERY SELECT fli_id,
                        ...
                        fli_loc_arrival
                 FROM public.Flight
                 WHERE fli_departuredate::date =
                       TO_TIMESTAMP(_departuredate, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone
                   and fli_arrivaldate::date =
                       TO_TIMESTAMP(_arrivaldate, 'MM-DD-YYYY HH24:MI:SS')::timestamp without time zone
                   and fli_loc_departure = _departure
                   and fli_loc_arrival = _arrival;
END;
...
CREATE OR REPLACE FUNCTION public.getflightsbylocation(_departure integer,
                                                       _arrival integer)
    RETURNS TABLE
            (
                id            integer,
...
BEGIN
    RETURN QUERY SELECT fli_id,
                        ...
                        fli_loc_arrival
                 FROM public.Flight
                 WHERE fli_loc_arrival = _arrival
                   AND fli_loc_departure = _departure;
END;
...
BEGIN
    RETURN QUERY SELECT role.*
                 FROM Role AS role,
                      User_Role
                 WHERE usr_rol_id = role.rol_id
                   AND usr_use_id = user_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SET Use_name        = name,
        ...
        Use_document_id = doc_id,
        Use_email       = email
    WHERE use_id = id RETURNING use_id INTO user_id;
    RETURN user_id;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION AddBaggage(_bag_res_fli_fk INTEGER,
...
DECLARE
    ret_id INTEGER;
BEGIN
    INSERT INTO Baggage(bag_id, bag_res_fli_fk, bag_res_cru_fk, bag_descr, bag_status)
    VALUES (default, _bag_res_fli_fk, _bag_res_cru_fk, _bag_descr, _bag_status) RETURNING bag_id INTO ret_id;
    RETURN ret_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    DELETE
    FROM Baggage
    WHERE (bag_id = _bag_id) returning bag_id into ret_id;
    return ret_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT H.HOT_ID,
                 ...
                 FROM Hotel AS H,
                      LOCATION L
                 WHERE H.hot_loc_fk = L.LOC_ID
                   and H.hot_loc_fk = city_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT L.LOC_ID, L.LOC_CITY, L.LOC_COUNTRY
                 FROM LOCATION L,
                      (select LOC_COUNTRY FROM LOCATION WHERE LOC_ID = city_id) AS L1
...
BEGIN
    ...
    DELETE
    FROM HOTEL
    WHERE (HOT_ID = _HOT_ID) returning HOT_ID into RET_ID;
    return RET_ID;
END;
$$ LANGUAGE plpgsql;
...
---------Agregar Ship-------------------
CREATE OR REPLACE FUNCTION AddShip(_shi_name VARCHAR(20),
...
BEGIN
    INSERT INTO Ship(shi_id, shi_name, shi_capacity, shi_loadingcap, shi_model,
                     shi_line, shi_picture)
    VALUES (default, _shi_name, _shi_capacity, _shi_loadingcap, _shi_model, _shi_line,
            _shi_picture) RETURNING shi_id INTO ret_id;
    RETURN ret_id;
END;
$$ LANGUAGE plpgsql;
-------Update Cruiser Status--------------------
CREATE OR REPLACE FUNCTION UpdateShipStatus(_shi_id integer,
                                            _status boolean)
    returns integer AS
$$
declare
...
CREATE OR REPLACE FUNCTION AddCruise(_cru_shi_fk INTEGER,
...
BEGIN
    ...
    INSERT INTO Cruise(cru_id, cru_shi_fk, cru_departuredate, cru_arrivaldate, cru_price,
                       cru_loc_arrival, cru_loc_departure)
    VALUES (default, _cru_shi_fk, TO_TIMESTAMP(_cru_departuredate, 'yyyy-mm-dd hh24:mi'),
            TO_TIMESTAMP(_cru_arrivaldate, 'yyyy-mm-dd hh24:mi'), _cru_price, _cru_loc_arrival,
            _cru_loc_departure) RETURNING cru_id INTO ret_id;
    RETURN ret_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    DELETE
    FROM Ship
    WHERE (shi_id = _shi_id) returning shi_id into ret_id;
    DELETE
    FROM Cruise
...
BEGIN
    ...
    DELETE
    FROM Cruise
    WHERE (cru_id = _cru_id) returning cru_id into ret_id;
    return ret_id;
END;
$$ LANGUAGE plpgsql;
--------Modificar Ship-------------------
CREATE OR REPLACE FUNCTION ModifyShip(_shi_id integer,
...
BEGIN
    ...
    UPDATE Ship
    SET shi_isactive   = _shi_isactive,
        shi_name       = _shi_name,
        shi_capacity   = _shi_capacity,
        shi_loadingcap = _shi_loadingcap,
        shi_model      = _shi_model,
        shi_line       = _shi_line,
        shi_picture    = _shi_picture
    WHERE (shi_id = _shi_id) returning shi_id into ret_id;
    return ret_id;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION ModifyCruise(_cru_id integer,
                                        _shi_id integer,
                                        _cru_departuredate VARCHAR,
...
BEGIN
    UPDATE Cruise
    SET cru_departuredate= TO_TIMESTAMP(_cru_departuredate, 'yyyy-mm-dd hh24:mi'),
        cru_shi_fk = _shi_id,
        cru_arrivaldate = TO_TIMESTAMP(_cru_arrivaldate, 'yyyy-mm-dd hh24:mi'),
        cru_loc_arrival = _loc_arrival,
        cru_loc_departure = _loc_departure,
...
BEGIN
    SELECT shi_isactive
    FROM ship
    INTO stat WHERE shi_id IN (SELECT cru_shi_fk FROM CRUISE
  WHERE cru_loc_departure = _dep_id
  AND cru_loc_arrival = _arr_id);
    IF stat = True THEN
        RETURN QUERY SELECT c.cru_id,
                            c.cru_shi_fk,
                            c.cru_departuredate,
                            c.cru_arrivaldate,
                            c.cru_price,
...
--------------------------------------AGREGAR RECLAMO----------------------------------
CREATE OR REPLACE FUNCTION AddClaim(_cla_title VARCHAR(20),
...
BEGIN
    ...
    INSERT INTO Claim(cla_title, cla_descr, cla_status)
    VALUES (_cla_title, _cla_description, 'ABIERTO') RETURNING cla_ID INTO _cla_ID;
...
----------------------------------------MODIFICAR RECLAMO -----------------------------
CREATE OR REPLACE FUNCTION ModifyClaimStatus(_cla_id integer,
...
CREATE OR REPLACE FUNCTION ModifyClaimTitle(_cla_id integer,
...
BEGIN
    ...
    UPDATE Claim
    SET cla_title= _cla_title,
        cla_descr= _cla_descr
    WHERE (cla_id = _cla_id);
    RETURN _cla_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT cla_id,
                 ...
                 from claim,
                      ...
                      res_cru,
                      users
                 where BAG_RES_CRU_FK = rc_id
                   and rc_use_fk = use_id
                 ...
                 from claim,
                      ...
                      res_fli,
                      users
                 where BAG_RES_fli_FK = rf_id
                   and rf_use_fk = use_id
                   and use_document_id = _users_document_id
                   and bag_cla_fk = cla_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT BAG_id,
                        ...
                        BAG_status
                 FROM Baggage
                 WHERE BAG_status = _bag_status;
END;
$$ LANGUAGE plpgsql;

---------------------------- MODIFICAR ESTATUS DE EQUIPAJE -----------------------
CREATE OR REPLACE FUNCTION modifyBaggagestatus(_bag_id integer,
...
-----------------------------------Grupo 5 ------------------------------------------------
CREATE OR REPLACE FUNCTION AddVehicle(modelId INTEGER,
                                      locationId INTEGER,
                                      license VARCHAR,
                                      price DECIMAL,
                                      status BOOLEAN)
...
CREATE OR REPLACE FUNCTION AddModel(brandId INTEGER,
                                    modelName VARCHAR,
                                    capacity INTEGER,
                                    picture VARCHAR)
    RETURNS INTEGER AS
...
CREATE OR REPLACE FUNCTION UpdateModel(vmid INTEGER,
                                       vmbrand INTEGER,
                                       vmname VARCHAR,
                                       vmcapacity INTEGER,
                                       vmpicture VARCHAR)
...
CREATE OR REPLACE FUNCTION UpdateBrand(vbid INTEGER,
                                       vbname VARCHAR)
    RETURNS BOOLEAN AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION UpdateVehicle(vehid INTEGER,
                                         vehmodel INTEGER,
                                         vehlocation INTEGER,
                                         vehlicense VARCHAR,
                                         vehprice DECIMAL,
...
CREATE OR REPLACE FUNCTION UpdateVehicleStatus(vehid INTEGER,
                                               vehstatus BOOLEAN)
    RETURNS BOOLEAN AS
$$
BEGIN
...
BEGIN
    RETURN QUERY
        ...
        FROM vehicle,
             veh_model AS Model,
             veh_brand AS Brand
        WHERE veh_model = Model.vm_id
          AND Model.vm_brand = Brand.vb_id
          AND Brand.vb_id = brandId;
END;
$$
    LANGUAGE 'plpgsql';
...
CREATE OR REPLACE FUNCTION ModifyRestaurant(id integer,
                                            name VARCHAR(100),
                                            capacity INTEGER,
                                            isActive BOOLEAN,
                                            qualify DECIMAL,
...
CREATE OR REPLACE FUNCTION AddTravel(travelName VARCHAR,
                                     travelInit VARCHAR,
                                     travelEnd VARCHAR,
...
BEGIN
    ...
    VALUES (travelName, to_date(travelInit, 'YYYY-MM-DD'), to_date(travelEnd, 'YYYY-MM-DD'), travelDescription,
            userId) RETURNING tra_id INTO travelId;
    RETURN travelId;
END;
$$
    LANGUAGE 'plpgsql';
...
CREATE OR REPLACE FUNCTION GetReservationsOfHotelByTravelAndLocation(travelId INTEGER,
                                                                     locationId INTEGER)
    RETURNS TABLE
            (
...
BEGIN
    RETURN QUERY
        ...
        WHERE tr.tr_travel_fk = travelId
          AND tl.tl_loc_fk = locationId
          AND h.hot_loc_fk = locationId;
END;
$$
    LANGUAGE plpgsql;

-- DROP FUNCTION updatetravel(integer,character varying,character varying,character varying,character varying);
CREATE OR REPLACE FUNCTION UpdateTravel(travelId INTEGER,
                                        travelName VARCHAR,
                                        travelDescription VARCHAR,
                                        travelInit VARCHAR,
                                        travelEnd VARCHAR)
...
BEGIN
    UPDATE travel
    SET tra_name  = travelName,
        tra_descr = travelDescription,
        tra_ini   = to_date(travelInit, 'YYYY-MM-DD'),
        tra_end   = to_date(travelEnd, 'YYYY-MM-DD')
...
CREATE OR REPLACE FUNCTION AddReservationToTravel(travelId INTEGER,
                                                  reservationId INTEGER,
...
BEGIN
    IF reservationType = 'HOTEL' THEN
        INSERT INTO TRA_RES (TR_TRAVEL_FK, TR_RES_CRU_FK, TR_RES_FLI_FK, TR_RES_REST_FK, TR_RES_AUT_FK, TR_RES_ROO_FK,
                             TR_TYPE)
...
$$
BEGIN
    RETURN QUERY select r.rf_id             as id,
                        f.fli_price,
                        f.fli_departuredate as salida,
                        r.rf_seatnum        as asiento,
                        l.loc_country       as pais_salida,
                        l.loc_city          as ciudad_salida,
                        f.fli_loc_arrival,
                        r.rf_num_ps
                 FROM res_fli r,
                      flight f,
                      ...
                      location l,
                      users u
                 where f.fli_id = r.rf_fli_fk
                   and r.rf_use_fk = usuario
                   and u.use_id = r.rf_use_fk
                   and p.pla_id = f.fli_pla_fk
                   and l.loc_id = f.fli_loc_departure;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    capacity Integer;
BEGIN
    SELECT p.pla_capacity
    FROM flight as f,
         plane as p
    WHERE f.fli_id = idflight
    into capacity;
    return capacity;
END;
$$
...
BEGIN
    RETURN QUERY SELECT loc_id,
                        ...
                        loc_country
                 FROM location
                 WHERE location.loc_id = _id_city;
END;
...
CREATE OR REPLACE FUNCTION public.GetFlightsIDAVU(_departure integer, _arrival integer, _departuredate char varying,
                                                  _arrivaldate char varying)
    RETURNS TABLE
            (
                id            integer,
...
BEGIN
    RETURN QUERY SELECT fli_id,
                 ...
                 FROM public.Flight
                 WHERE fli_departuredate::date = _departuredate::timestamp without time zone
                   and fli_arrivaldate::date = _arrivaldate::timestamp without time zone
                   and fli_loc_departure = _departure
                   and fli_loc_arrival = _arrival;
END;
...
CREATE OR REPLACE FUNCTION addReservationRestaurant(fecha VARCHAR,
                                                    people INTEGER, fecha_reservacion VARCHAR, userId INTEGER,
...
BEGIN
    INSERT INTO res_rest(rr_date, RR_NUM_PPL, rr_timestamp, rr_use_fk,
                         rr_res_fk)
    VALUES (TO_TIMESTAMP(fecha, 'yyyy-mm-dd hh24:mi'), people, TO_TIMESTAMP(fecha_reservacion, 'yyyy-mm-dd hh24:mi'),
            userId, restaurantId) RETURNING rr_id INTO res_rest_id;
...
CREATE OR REPLACE FUNCTION getResRestaurant(usuario INTEGER)
    RETURNS TABLE
            (
                id                INTEGER,
                ciudad            VARCHAR,
...
BEGIN
    RETURN QUERY select m.rr_id, l.loc_city, l.loc_country, r.res_name, r.res_address_specs, m.rr_date, m.rr_num_ppl
                 FROM restaurant r,
                      ...
                      location l,
                      users u
                 where m.rr_use_fk = usuario
                   and r.res_id = m.rr_res_fk
                   and l.loc_id = r.res_loc_fk
                   and U.USE_ID = m.rr_use_fk;

END;
$$ LANGUAGE plpgsql;

--SP para retornar aquellas reservas que no han sido pagadas
CREATE OR REPLACE FUNCTION getReservationNotPay(usuario INTEGER)
    RETURNS TABLE
            (
                id        INTEGER,
                f_reserva TIMESTAMP
...
BEGIN
    RETURN QUERY select r.rr_id, r.rr_date
                 FROM res_rest r,
                      users u
                 where r.rr_use_fk = usuario
                   and u.use_id = rr_use_fk
                   and rr_pay_fk isNull;
END;
$$ LANGUAGE plpgsql;
...
------Grupo1-----------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION LoginRepository(Email varchar(20), Password VARCHAR(50))
    RETURNS table
            (
                use_id        integer,
                use_name      varchar(50),
                use_last_name varchar(30),
                usr_rol_id    integer,
                rol_name      varchar(30)
            )
AS
...
BEGIN
    RETURN QUERY
        select USERS.use_id, USERS.use_name, USERS.use_last_name, User_Role.usr_rol_id, Role.rol_name
        from USERS,
             User_Role,
             Role
        WHERE USERS.use_email = $1
          and (USERS.use_password = MD5($2) or
               USERS.use_password = $2)
          and USERS.use_id = User_Role.usr_use_id
          and Role.rol_id = User_Role.usr_rol_id;
END;
$BODY$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION RecoveryPass(Email varchar(20))
    RETURNS table
            (
                use_name     varchar(50),
...
CREATE OR REPLACE FUNCTION public.m13_addautomobilereservation(_checkin timestamp without time zone,
...
CREATE OR REPLACE FUNCTION public.m13_getresrooms(
    ...
    RETURNS TABLE
            (
                rr_id           integer,
                rr_checkindate  timestamp without time zone,
                rr_checkoutdate timestamp without time zone,
                rr_timestamp    timestamp without time zone,
                rr_hot_fk       integer,
                rr_use_fk       integer,
                rr_pay_fk       integer,
                hot_id          integer
...
CREATE OR REPLACE FUNCTION public.m13_findbyroomreservationid(
    ...
    RETURNS TABLE
            (
                res_roo_id            integer,
                res_roo_fecha_ingreso timestamp without time zone,
                res_roo_fecha_salida  timestamp without time zone,
                rr_timestamp          timestamp without time zone,
                rr_hot_fk             integer,
                rr_use_fk             integer,
                rr_pay_fk             integer
...
CREATE OR REPLACE FUNCTION public.m13_addroomreservation(_checkin timestamp without time zone,
                                                         _checkout timestamp without time zone,
                                                         _use_fk integer,
                                                         _rr_hot_fk integer)
    RETURNS integer
...
BEGIN
    ...
    VALUES (_Checkin, _Checkout, CURRENT_TIMESTAMP, _use_fk, _rr_hot_fk) RETURNING rr_id into _id;
    return _id;
END;
$BODY$;
...
CREATE OR REPLACE FUNCTION public.m13_getresroobyuserandroomid(
    ...
    RETURNS TABLE
            (
                ...
                rr_checkoutdate   timestamp without time zone,
                rr_hot_fk         integer,
                hot_name          character varying,
                hot_room_capacity integer,
                hot_room_price    numeric,
                hot_phone         character varying,
                hot_stars         integer
            )
    LANGUAGE 'plpgsql'
...
ALTER FUNCTION public.m13_getresroobyuserandroomid(integer)
    OWNER TO vacanza;
--AVAILABILITY HOTEL
CREATE OR REPLACE FUNCTION getAvailableRoomsBasedOnReservationByHotelId(IN _hot_id integer, OUT disponibles int)
AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION m13_modifyReservationRoomPayment(pay Integer, reservation Integer)
    RETURNS
        INTEGER
AS
$$
...
CREATE OR REPLACE FUNCTION m13_modifyReservationAutomobilePayment(pay Integer, reservation Integer)
    RETURNS
        INTEGER
AS
$$
...
DECLARE
    id integer;
BEGIN
    INSERT INTO public.payment(pay_method,
                               pay_total,
                               pay_timestamp)
    VALUES (_paymetMethod,
            _pay_total,
            CURRENT_TIMESTAMP) RETURNING pay_id INTO id;
    RETURN id;
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION getinfoorder(IN _id bigint,
                                        IN _tipo integer)
    RETURNS TABLE
            (
                id          integer,
...
CREATE OR REPLACE FUNCTION getinfoorderAll(IN _idAuto bigint,
                                           IN _idRoo bigint,
                                           IN _idRes bigint,
                                           IN _idCru bigint)
    RETURNS TABLE
...
CREATE OR REPLACE FUNCTION getNoPaysResAutHab(IN _id bigint,
                                              IN _tipo integer)
    RETURNS TABLE
            (
                id    integer,
...
CREATE OR REPLACE FUNCTION PutPayProccess(IN _idPay bigint,
                                          IN _idAuto bigint,
                                          IN _idRoo bigint,
                                          IN _idRes bigint,
                                          IN _idCru bigint)
